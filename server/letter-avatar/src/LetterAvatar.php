...
class LetterAvatar
{
    ...
    public function generate()
    {
        ...
        $len = count($words);
        if ($len === 1) {
            $this->name_initials = mb_strtoupper(trim(mb_substr($words[0], 0, 1, 'UTF-8')));
        }
        else if ($len > 1) {
            $this->name_initials = mb_strtoupper(trim(mb_substr($words[$len-2], 0, 1, 'UTF-8'))) . mb_strtoupper(trim(mb_substr($words[$len-1], 0, 1, 'UTF-8')));
        }
    ...
    public function getPng($mimetype = 'image/png', $quality = 90)
    {
